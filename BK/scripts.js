$( document ).ready(function() {
	...
	var map = L.mapbox.map('map', 'davidhalfon.m49k783f').featureLayer.setGeoJSON(geojson);
			...
			new function(){ // search handler
				...
				if(check == 1){ // if value has been send from user
					...
					for(var i = 0; i < geojson[0].features.length; i++){	
						...
						if (value == geojson[0].features[i].properties.Country){
							$("#radio").attr("src", geojson[0].features[i].properties.Url);
							$(".player").show();
							$("#1").html("Station: "+geojson[0].features[i].properties.Station);
							$("#2").html("Country: "+geojson[0].features[i].properties.Country);
							$("#3").html("City: " +geojson[0].features[i].properties.City);
